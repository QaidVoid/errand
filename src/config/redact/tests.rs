...
fn the_configuration_can_be_logged_with_every_secret_field_blanked() {
    ...
    assert_eq!(shown.matches(REDACTION).count(), SECRET_PATHS.len());
}
...
fn a_value_too_short_to_be_a_credential_is_not_scrubbed_from_prose() {
    ...
    assert_eq!(
        secret_values(&shortened),
        vec![CREDENTIAL.to_owned(), GITHUB_TOKEN.to_owned()]
    );
}
